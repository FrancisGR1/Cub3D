...
test_should_fail() {
	...
	#1st run -> catch stderr
	local found_error=$(./"${EXECUTABLE}" "$map_path" 2>&1 >/dev/null)
	local has_correct_error=false
	if [ "$found_error" = "$EXPECTED_ERROR_MSG" ]; then
		has_correct_error=true
	fi
	...
	if [ -n "$found_error" ] && [ "$has_correct_error" = true ]  && [ $exit_code -eq 1 ] && [ $bytes_not_freed -eq 0 ]; then
	 	printf "${GREEN}[OK][OK][OK]${RESET}\t$map_to_test\n"
		rm $val_out_file
		TESTS_PASSED=$((TESTS_PASSED + 1))
	else
		...
		# so print invidually
		# for better diagnostics
		if [ -z "$found_error" ] || [ "$has_correct_error" = false ];  then
			printf "${RED}[KO]${RESET}"
		else
			printf "${GREEN}[OK]${RESET}"
		fi
		...
		if [ -z "$found_error" ] || [ "$has_correct_error" = false ];  then
			...
			if [ "$has_correct_error" = false ]; then
				printf "${EXECUTABLE} said $found_error instead of ${EXPECTED_ERROR_MSG}\n"
			fi
		fi
		if [ $exit_code -ne 1 ]; then
			printf "exit code: $exit_code\n"
...
test_should_pass "map_tabs_in_each_line.cub"
...
test_should_fail "map_non_binary_nums"
test_should_fail "map_negative_nums"
